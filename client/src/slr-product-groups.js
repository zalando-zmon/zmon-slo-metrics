...
import './slr-fab.js';
import './slr-product-group-card.js';
import './slr-ajax.js';
import './slr-items-data.js';
import { html } from '@polymer/polymer/lib/utils/html-tag.js';
import { afterNextRender } from '@polymer/polymer/lib/utils/render-status.js';
import { dom } from '@polymer/polymer/lib/legacy/polymer.dom.js';
class SlrProductGroups extends PolymerElement {
  static get template() {
    return html`
    ...
    <slr-ajax id="ajax" url="[[selectedProductGroup.uri]]" method="DELETE" handle-as="json" on-response="handleResponse" on-error="handleErrorResponse" debounce-duration="300" loading="{{loading}}"></slr-ajax>

    <slr-items-data id="productGroupsData" items="{{productGroups}}" url="/api/product-groups" search="[[searchStr]]" item-type="Product Group" loading="{{loading}}" pagination-enabled="true"></slr-items-data>
    ...
    <paper-dialog id="delete">
      ...
      <div class="buttons">
        ...
        <paper-button dialog-confirm="" on-click="deleteProductGroup">Delete</paper-button>
      </div>
    </paper-dialog>

    <iron-scroll-threshold id="scroll" scroll-target="document" lower-threshold="500" on-lower-threshold="onLowerScrollThreshold">
      <div class="flex-grid-thirds" small-layout\$="{{small}}" id="list">
        <template is="dom-repeat" items="[[productGroups]]">
          <div class="col productGroup" small-layout\$="{{small}}">
            <slr-product-group-card id="card{{index}}" item="{{item}}" loading="{{loading}}"></slr-product-group-card>
          </div>
        </template>
      </div>
    </iron-scroll-threshold>

    <slr-fab id="fab" on-click="addProductGroup" visible=""></slr-fab>
`;
  }
  ...
  addProductGroup() {
    ...
    afterNextRender(this, () => {
      ...
      let el = dom(this.root).querySelector('#'+ id)
      el.scrollIntoView()
    })
  }
