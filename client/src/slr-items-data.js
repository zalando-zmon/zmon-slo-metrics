...
class SlrItemsData extends PolymerElement {
  static get template() {
    return html`
    <slr-ajax auto="{{auto}}" id="ajax" url="{{url}}" params="{{params}}" loading="{{loading}}" handle-as="json" on-response="handleResponse" on-error="handleErrorResponse" reject-with-request="true" debounce-duration="300"></slr-ajax>
`;
  }
